...
server(Master, MaxPrp, MaxAgr, Nodes, Cast, Queue, Jitter, Cont) ->
receive
    {send, Msg} ->
        Ref = make_ref(),
        request(Ref, Msg, Nodes, Jitter),
        NewCast = cast(Ref , Nodes , Cast), 
        server(Master, MaxPrp, MaxAgr, Nodes, NewCast, Queue, Jitter, Cont);
    {request, From, Ref, Msg} ->
        ...
        server(Master, NewMaxPrp, MaxAgr, Nodes, Cast, NewQueue, Jitter, Cont+1);
    {proposal, Ref, Proposal} ->
        case proposal(Ref, Proposal , Cast) of
            {agreed, MaxSeq, NewCast} ->
                agree(Ref, MaxSeq, Nodes),
                server(Master, MaxPrp, MaxSeq, Nodes, NewCast, Queue, Jitter, Cont+1);
            NewCast ->
                server(Master, MaxPrp, MaxAgr, Nodes, NewCast, Queue, Jitter, Cont+1)
        end;
    {agreed, Ref, Seq} ->
        ...
        server(Master, MaxPrp, NewMaxAgr, Nodes, Cast, NewQueue, Jitter, Cont+1);
    stop ->
		io:format("Numero de mensajes multicast: ~w ~n", [Cont]),
        ok
end.
